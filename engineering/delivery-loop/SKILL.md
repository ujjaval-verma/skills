...
- **The orchestrator holds only**: the DOD input, the queue, per-slice gate verdicts, the regression baseline, and one-paragraph summaries of shipped slices. It reads subagent reports; it does not read slice diffs.
